...
async def repDown(client, message):
    try:
        if message.reply_to_message.from_user.is_self:
            if Path(f"MrMozozavrbotik/myuserbot123321/reputation.txt").is_file():
                with open("reputation.txt", "r") as f:
                    NowReputation = int(f.read())
                    f.close()
            else:
                NowReputation = 0
            with open("MrMozozavrbotik/myuserbot123321/reputation.txt", "w") as f:
...
async def repUp(client, message):
    try:
        if message.reply_to_message.from_user.is_self:
            if Path(f"MrMozozavrbotik/myuserbot123321/reputation.txt").is_file():
                with open("MrMozozavrbotik/myuserbot123321/reputation.txt", "r") as f:
                    NowReputation = int(f.read())
                    f.close()
            else:
                NowReputation = 0
